...
const { mockFindByUsername, mockVerifyPassword, mockCreateSession, mockJson, mockRedirect } = vi.hoisted(() => ({
  ...
  mockCreateSession: vi.fn(() => ({ name: 'session', value: 'token', options: { httpOnly: true, path: '/' } })),
...
vi.mock('@/lib/db/auth', () => ({
  DUMMY_HASH: '$2a$12$dummydummydummydummydummydummydummydummydummydummydu',
...
describe('POST /api/auth/login', () => {
  ...
  it('redirects to workbench for form login success', async () => {
    ...
    expect(res.url).toBe('http://localhost/portal');
    expect(mockCreateSession).toHaveBeenCalledWith({
      id: 'u1',
      username: 'testuser',
      platformRole: 'user',
  ...
  it('redirects form login success to the browser host for LAN access', async () => {
    ...
    expect(res.status).toBe(303);
    expect(res.url).toBe('http://172.17.137.235:3000/portal');
  });
});
