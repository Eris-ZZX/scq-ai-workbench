...
describeDatabase('admin activity structure save regressions', () => {
  beforeAll(async () => {
    ...
    await db.stageGateRecord.create({
      data: { projectId, stage: 'TR1' },
    });

    await db.projectActivityParent.create({
      data: {
        id: existingParentId,
        projectId,
        stage: 'TR1',
    ...
    await db.projectActivityChild.create({
      data: {
        id: existingChildId,
        projectId,
  ...
  afterAll(async () => {
    ...
    await db.activityTemplateSet.deleteMany({ where: { id: templateSetId } });
  ...
  it('template center saves edited activity structure as a new published version', async () => {
    ...
    const set = await db.activityTemplateSet.findUniqueOrThrow({
    ...
    const stages = await db.activityTemplateStage.findMany({
  ...
  it('project management preserves an added empty stage after saving activity structure', async () => {
    ...
    const gate = await db.stageGateRecord.findUnique({
