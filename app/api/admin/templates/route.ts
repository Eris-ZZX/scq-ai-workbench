...
async function getTemplateCenterView() {
  const sets = await db.activityTemplateSet.findMany({
    include: {
      versions: {
  ...
  const stages = activeVersionIds.length
    ? await db.activityTemplateStage.findMany({
  ...
  const parents = stageIds.length
    ? await db.activityTemplateParent.findMany({
  ...
  const children = parentIds.length
    ? (await Promise.all(chunk(parentIds, 400).map((ids) => db.activityTemplateChild.findMany({
  ...
  return sets.map((set) => {
    ...
    const versions = set.versions.map((version: any) => ({
      ...version,
      stages: version.id === activeVersionId ? stagesByVersionId.get(version.id) ?? [] : [],
    }));
    const version = versions.find((item: any) => item.id === set.latestPublishedVersionId) ?? versions[0];
    const viewStages = version?.stages ?? [];
    const parentCount = viewStages.reduce((sum: number, stage: any) => sum + stage.parents.length, 0);
    const childCount = viewStages.reduce(
      (sum: number, stage: any) => sum + stage.parents.reduce((inner: number, parent: any) => inner + parent.children.length, 0),
      0,
    );
...
export async function DELETE(request: Request) {
  ...
  try {
    await db.$transaction(async (tx) => {
      ...
      const versionIds = set.versions.map((version: any) => version.id);
      await tx.activityTemplateSet.update({
        where: { id },
