...
export async function POST(request: Request) {
  ...
  const existing = await db.projectRole.findUnique({ where: { code }, select: { id: true } });
  if (existing) return NextResponse.json({ error: '角色标识已存在' }, { status: 409 });

  const count = await db.projectRole.count();
...
export async function DELETE(request: Request) {
  ...
  const role = await db.projectRole.findUnique({ where: { id }, select: { code: true } });
  if (!role) return NextResponse.json({ error: '角色不存在' }, { status: 404 });

  const memberCount = await db.projectMember.count({ where: { assignedRole: role.code } });
  if (memberCount > 0) {
