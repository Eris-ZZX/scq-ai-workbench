...
import { db, isForeignKeyViolation, isRecordNotFound } from '@/lib/database';
...
export async function POST(request: Request) {
  ...
  const selectedTemplate = activityTemplateSetId
    ? await db.activityTemplateSet.findFirst({
  ...
  try {
    ...
    const project = await db.project.findUnique({ where: { id: created.id }, select: projectSelect() });
  ...
  try {
      ...
      if (userIds.length > 0) {
        ...
        await db.$transaction(async (tx) => {
          ...
          for (const userId of userIds) {
            ...
            if (!m) continue;
              const currentRoles = m.assignedRole ? m.assignedRole.split(',').map((s: string) => s.trim()).filter(Boolean) : [];
    ...
    if (action === 'removeMember') {
      ...
      if (roleName && member.assignedRole) {
          const roles = member.assignedRole.split(',').map((s: string) => s.trim()).filter(Boolean);
          const remaining = roles.filter((r: string) => r !== roleName);
        if (remaining.length === 0) {
          // 最后一个角色 → 删除成员
          if (member.role === 'owner') {
            const ownerCount = await db.projectMember.count({ where: { projectId, role: 'owner' } });
            if (ownerCount <= 1) {
    ...
    const project = await db.project.findUnique({ where: { id: projectId }, select: projectSelect() });
