...
function cloneStages(stages: TemplateStage[]): TemplateStage[] {
...
export default function AdminTemplatesPage() {
  ...
  function beginEdit() {
    ...
    setDraft({
      ...
      stages: cloneStages(selectedVersion.stages),
    });
    setEditMode(true);
  }
  ...
  function updateStage(stageId: string, fields: Partial<Pick<TemplateStage, 'code' | 'name'>>) {
    updateDraft((current) => ({
      ...current,
      stages: current.stages.map((stage) => (stage.id === stageId ? { ...stage, ...fields } : stage)),
    }));
  }
  ...
  function openEditStage(stage: TemplateStage) {
  ...
  function openEditParent(stageId: string, parent: TemplateParent) {
  ...
  function updateChild(stageId: string, parentId: string, childId: string, fields: Partial<TemplateChild>) {
    updateDraft((current) => ({
      ...current,
      stages: current.stages.map((stage) => stage.id === stageId
        ? {
  ...
  function openEditChild(stageId: string, parentId: string, child: TemplateChild) {
    setItemDialog({
      kind: 'child',
      mode: 'edit',
      stageId,
  ...
  function submitItemDialog() {
    ...
    if (itemDialog.kind === 'stage') {
      ...
      if (itemDialog.mode === 'edit' && itemDialog.stageId) {
        updateStage(itemDialog.stageId, { code: name, name });
      } else {
        updateDraft((current) => ({
          ...current,
          stages: [
    ...
    if (itemDialog.kind === 'parent') {
      ...
      if (itemDialog.mode === 'edit' && itemDialog.parentId) {
        ...
        updateDraft((current) => ({
          ...current,
          stages: current.stages.map((stage) => stage.id === itemDialog.stageId
            ? {
              ...stage,
              parents: [
                ...stage.parents,
    ...
    const fields = {
      title,
      roleGroup: itemDialog.roleGroup,
      ownerRoleName: itemDialog.roleGroup,
            ...
            <main className="min-w-0 rounded-lg border border-border bg-white">
              ...
              <div className="max-h-[calc(100vh-170px)] overflow-auto p-4">
                <div className="space-y-3">
                  ...
                  {visibleStages.length === 0 && <div className="rounded-lg border border-dashed border-border px-4 py-10 text-center text-sm text-muted-foreground">暂无阶段</div>}
                </div>
              </div>
            </main>
          </div>
              ...
              {itemDialog.kind === 'child' && (
                <>
                  ...
                  <label className="block text-xs font-medium text-muted-foreground">
                    ...
                    <select
                      className={fieldClass('mt-1 h-9 w-full')}
                      value={itemDialog.roleGroup}
                      onChange={(event) => setItemDialog((current) => (current?.kind === 'child' ? { ...current, roleGroup: event.target.value } : current))}
                    >
