...
export async function GET() {
  try {
    ...
    const tags = await prisma.aiResourceFavoriteTag.findMany({
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existing = await prisma.aiResourceFavoriteTag.findUnique({
    ...
    const max = await prisma.aiResourceFavoriteTag.aggregate({
      where: { userId: actor.userId },
      _max: { sortOrder: true },
    });

    const tag = await prisma.aiResourceFavoriteTag.create({
      data: {
        userId: actor.userId,
        name,
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if ('orderedTagIds' in payload.data) {
      ...
      await prisma.$transaction(
        orderedTagIds.map((id, index) =>
          prisma.aiResourceFavoriteTag.update({
            where: { id },
    ...
    const tag = await prisma.aiResourceFavoriteTag.findFirst({
    ...
    const conflict = await prisma.aiResourceFavoriteTag.findFirst({
    ...
    const updated = await prisma.aiResourceFavoriteTag.update({
      where: { id },
      data: { name },
      select: { id: true, name: true, sortOrder: true },
    });
