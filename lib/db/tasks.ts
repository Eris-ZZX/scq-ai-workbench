...
export async function createTask(params: {
  ...
  if (params.assigneeMemberId) {
    const am = await db.projectMember.findUnique({
  ...
  if (params.stageId) {
    const st = await db.projectStage.findUnique({
...
export async function updateTask(taskId: string, data: {
  ...
  if (data.assigneeMemberId) {
    ...
    if (task) {
      const am = await db.projectMember.findUnique({ where: { id: data.assigneeMemberId }, select: { projectId: true } });
  ...
  if (data.stageId) {
    const task = await db.task.findUnique({ where: { id: taskId }, select: { projectId: true } });
    if (task) {
      const st = await db.projectStage.findUnique({ where: { id: data.stageId }, select: { projectId: true } });
...
export function getTaskHistory(taskId: string) {
  return db.taskStatusChange.findMany({ where: { taskId }, orderBy: { createdAt: 'desc' } });
}
