...
type ActivityTx = DatabaseClient;
...
export async function ensureProjectActivities(projectId: string, actorUserId?: string, templateSetId?: string) {
  const existing = await db.projectActivityParent.count({ where: { projectId } });
  ...
  return db.$transaction(async (tx) => {
    ...
    for (const template of templates as ActivityTemplateLike[]) {
      ...
      if (!parent) {
        ...
        parent = createdParent as { id: string; sortOrder: number };
...
export async function getProjectActivityView(projectId: string, options?: { attachmentMode?: 'summary' | 'detail' }) {
  ...
  return db.projectActivityParent.findMany({
    where: { projectId },
    include: {
      children: {
        include: {
...
export async function refreshParentSummary(tx: ActivityTx, parentId: string) {
  ...
  const activeChildren = parent.children.filter((child: any) => !isChildEffectivelyCompleted(child));
  const hasBlocked = activeChildren.some((child: any) => child.isBlocked);
  const hasOverdue = activeChildren.some((child: any) => {
  ...
  } else if (parent.status === PARENT_STATUS.IN_PROGRESS || parent.children.some((child: any) => child.status !== CHILD_STATUS.NOT_STARTED || child.isNotApplicable)) {
...
export async function getActivityDashboard(projectId?: string) {
  ...
  const parents = await db.projectActivityParent.findMany({
    where,
    include: { children: true },
  });
