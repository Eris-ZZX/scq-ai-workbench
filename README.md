...
当前图纸可靠性应用 `sqm-drawing-reliability` 继续兼容 `/sqm/drawing-reliability` 入口和旧管理 API。外挂 SSO 地址支持 HTTP 和 HTTPS，生产环境的服务端连接测试也允许本机及内网地址；用户实际跳转地址仍需禁止账号、密码、查询参数和片段。该配置会扩大服务端请求范围并带来 SSRF 风险，仅应在受信任的管理员可配置环境中使用。
